	 ...
	 private class DownloadFilesTask extends AsyncTask<URL, Integer, Long> {
	     protected Long doInBackground(URL... urls) {
     	    ...
     	    HttpGet httpget = new HttpGet( edURL.getText().toString() + 
     	    		"/get?molecule=1SUI&scheme=0&postprocessing=0&bkcolor=255,255,255&structure=0&size=0&quality=1" +
     	    		"&rotation=" + viewAngles.x + "," + viewAngles.y + ",0&scene=0&distance=-5000&depth=50000&fake=0&values=0,0");
        ...
        public boolean onTouch(View view, MotionEvent motionEvent) {
        	if( !renderingInProgress )
        	{
	        	...
	        	if( motionEvent.getAction() == MotionEvent.ACTION_DOWN )
	        	{
	        		...
	        		URL url;
					try {
						url = new URL("http://192.168.1.3:10000/get?"); // To remove
		        		task.execute(url);
